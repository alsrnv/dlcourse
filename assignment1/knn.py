...
class KNN:
    ...
    def compute_distances_two_loops(self, X):
        ...
        for i_test in range(num_test):
            for i_train in range(num_train):
                ...
                #pass
                #dists[i_test, i_train] = np.sqrt(np.sum((X[i_test]-self.train_X[i_train])))
    ...
    def compute_distances_no_loops(self, X):
        ...
        #dists = np.sum((X**2).sum(axis=1)[:, np.newaxis] + (self.train_X**2).sum(axis=1) - 2 * X.dot(self.train_X.T))
        #dists = np.sum(np.sum(X, axis = 1)[:, np.newaxis] - np.sum(self.train_X, axis = 1), axis = 1 )
    ...
    def predict_labels_binary(self, dists):
        ...
        for i in range(num_test):
            ...
            closest_y = self.train_y[y_indicies]
